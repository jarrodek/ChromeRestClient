...
public interface ChromeRuntime {
	...
	public interface RuntimeUpdateCheckHandler {
		/**
		 * 
		 * @param status Result of the update check. One of "throttled", "no_update", or "update_available"
		 * @param version Optional, if an update is available, this contains more information about the available update. Null otherwise.
		 */
	 ...
	 * This API is available since Chrome 41.
	 ...
	 * @param handler Called when the uninstall URL is set. If the given URL is invalid, runtime.lastError will be set.
	 */
	 ...
	 * @param extensionId Optional. The ID of the extension/app to send the message to. If omitted, the message will be sent to your own extension/app. Required if sending messages from a web page for web messaging.
	 * @param message
	 * @param options Optional. "includeTlsChannelId" (boolean) Whether the TLS channel ID will be passed into onMessageExternal for processes that are listening for the connection event.
	 * @param handler The JSON response object sent by the handler of the message.
	 */
//	void sendMessage(String extensionId, Object message, JavaScriptObject options, RuntimeJsonObjectHandler handler);
	/**
	 * Returns information about the current platform.
	 * @param handler Called with results
	 */

 ...
 */
public class RestClientApp implements EntryPoint, ResizeHandler,
	...
	@UiField(provided = true)
	HistoryListWidget historyList;
	@UiField(provided = true)
	RestoreStateWidget restoreWidget;
	@UiField(provided = true)
	...
	private void afterTaskFinishedHandler() {
		plusButton
				.setHTML("<g:plusone href=\"https://chrome.google.com/webstore/detail/hgmloofddffdnphfgcellkdfbfbjeloo\"></g:plusone>");
	...
	private void registerHandlers() {
		...
		SaveRequestEvent.register(eventBus, new SaveRequestEvent.Handler() {
			...
			public void onSaveAction(Object source) {
				RequestSaver.saveCurrentAction();
			}
		});
		RequestSaver.registerOnRemoveFormHandler();
		...
		FormStateSaveEvent.register(eventBus, new FormStateSaveEvent.Handler() {
			...
			public void onSave(final String formName, Object source) {
				RequestSaver.saveCurrentState(formName);
			}
		});

		RestoreRequestEvent.register(eventBus,
				new RestoreRequestEvent.Handler() {
					@Override
					public void onRestoreAction(RestForm form, Object source) {
						RequestSaver.isRestoredRequest = true;
						RequestSaver.currentRestoredRequestId = form.getId();
						RequestSaver.currentRestoredRequestName = form
								.getName();
		...
		HistoryRestoreEvent.register(eventBus,
				new HistoryRestoreEvent.Handler() {
					@Override
					public void onRestoreAction(RequestHistoryItem form,
							Object source) {
		...
		//
		// Below this point register events depends of config state from
		// localStorage
		//
		...
		String cookiesValue = storage
				.getItem(RestApp.StorageKeys.COOKIES_CAPTURE);
	...
	/**
	 * History menu item can be turned off. This method access this item visible
	 * state.
	 * 
	 * @param visible
	 */
	private void setHistoryMenuItemVisible(boolean visible) {
		...
		HTML5Element historyMenuItem = root.querySelector("ul."
				+ style.mainNav() + " > li[page=\"history\"]");
	...
	public void onResize(ResizeEvent event) {
		...
		if (history > 0 && windowHeight >= history) {
			...
			historyPanel.getElement().getStyle()
					.setProperty("minHeight", windowHeight + "px");
		} else {
	...
	/**
	 * Close current opened tab if any. It's also remove selection from menu.
	 */
	private void closeCurrentTab(HTML5Element root) {
		HTML5Element currentMenu = root.querySelector("ul." + style.mainNav()
		...
		final HTML5Element current = root.querySelector("div."
				+ style.subpage() + ":not(.hidden)");
		if (current == null) {
			return;
		}
	...
	/**
	 * Show semi-transparent div on main request panel to lock UI before show
	 * sub panel
	 * 
	 * @param root
	 */
	private void showBackDrop(HTML5Element root) {
		final HTML5Element subpageBackdrop = root.querySelector("div."
				+ style.subpageBackdrop() + "[hidden]");
	...
	private void hideBackDrop(HTML5Element root) {
		final HTML5Element subpageBackdrop = root.querySelector("div."
				+ style.subpageBackdrop() + ":not(.transparent)");
	...
	public void onValueChange(ValueChangeEvent<String> event) {
		...
		HTML5Element toSelect = root.querySelector("ul." + style.mainNav()
				+ " > li[page=\"" + token + "\"]");
		if (toSelect != null) {
		...
		if (token.equals("saved")) {
			...
			new Timer() {
				@Override
				public void run() {
					savedPanel.getParent().getElement()
							.removeClassName("transparent");
				}
			}.schedule(10);
			...
			new Timer() {
				@Override
				public void run() {
					aboutPanel.getParent().getElement()
							.removeClassName("transparent");
				}
			}.schedule(10);
			...
			new Timer() {
				@Override
				public void run() {
					historyPanel.getParent().getElement()
							.removeClassName("transparent");
				}
			}.schedule(10);
			...
			onResize(null);
			if (importUID != null) {
				ImportExternalDataAction action = new ImportExternalDataAction(
						importUID);
			...
			new Timer() {
				@Override
				public void run() {
					optionsPanel.getParent().getElement()
							.removeClassName("transparent");
				}
			}.schedule(10);
		}

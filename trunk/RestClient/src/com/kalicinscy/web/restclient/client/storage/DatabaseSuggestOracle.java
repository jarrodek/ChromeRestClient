...
import com.kalicinscy.web.restclient.client.storage.oracle.DatabaseHeadersResponse;
import com.kalicinscy.web.restclient.client.storage.oracle.DatabaseResponse;
import com.kalicinscy.web.restclient.client.storage.oracle.DatabaseUrlResponse;
import com.kalicinscy.web.restclient.client.storage.oracle.HeaderSuggestion;
import com.kalicinscy.web.restclient.client.storage.oracle.UrlSuggestion;
...
public class DatabaseSuggestOracle extends SuggestOracle {
   ...
   * the logic that uses the isComplete method.
   * query
   */
  ...
  /**
   * Return some suggestions to the SuggestBox. At this point we know that there
   * is no call to the server currently in progress and we try to satisfy the
   * request from the most recent results from the server before we call the
   * server.
   * 
    ...
    if (recentDatabaseResult != null) {
      if (mostRecentQuery.equals(recentDatabaseResult.getQuery())) {
        Response resp = new Response(recentDatabaseResult.filter(
            recentClientRequest.getQuery(), recentClientRequest.getLimit()));
        callback.onSuggestionsReady(recentClientRequest, resp);
      } else if (recentDatabaseResult.isComplete()
          && mostRecentQuery.startsWith(recentDatabaseResult.getQuery())) {
        Response resp = new Response(recentDatabaseResult.filter(
            recentClientRequest.getQuery(), recentClientRequest.getLimit()));
        callback.onSuggestionsReady(recentClientRequest, resp);
      } else {
  ...
  private void queryHeaders(final Request request, final Callback callback){
    ...
    ((HeadersService)databaseService).getNames(query, new ListCallback<HeaderRow>() {
      ...
      public void onSuccess(List<HeaderRow> result) {
        ...
        recentDatabaseResult = new DatabaseHeadersResponse(request,numberOfDatabaseSuggestions, suggestions);
  ...
  private void queryUrls(final Request request, final Callback callback){
    String query = request.getQuery();
    query = "%" + query + "%";
    ((UrlsService)databaseService).getUrls(query, new ListCallback<UrlRow>() {
      @Override
      public void onFailure(DataServiceException error) {
        requestInProgress = false;
      }

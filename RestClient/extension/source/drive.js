 ...
 * Initalize Google Drive library.
 */
...
function checkDriveAuth(callback) {
	...
	gapi.auth.authorize({'client_id': CLIENT_ID, 'scope': SCOPES, 'immediate': true}, handleDriveAuthResult);
}
function gdriveAuth(callback){
	window.setTimeout(function(){
		window.__arc_authcallback.push(callback);
		gapi.auth.authorize({'client_id': CLIENT_ID, 'scope': SCOPES, 'immediate': false}, handleDriveAuthResult);
	},250);
}
function handleDriveAuthResult(authResult) {
	console.log('handleDriveAuthResult',authResult, this);
	if(!window.__arc_authcallback || window.__arc_authcallback.length == 0) return;
...
function handlePickerLoad() {
	google.load("picker", "1", {"callback" : function(){
		...
		if(!window.__arc_pickercallback || window.__arc_pickercallback.length == 0) return;

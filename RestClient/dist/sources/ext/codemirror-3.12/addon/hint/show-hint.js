CodeMirror.showHint = function(cm, getHints, options) {
  ...
  function showHints(data) {
    ...
    var hints = document.createElement('paper-material'),
      selectedHint = 0;
    ...
    for (var i = 0; i < completions.length; ++i) {
      ...
      var className = 'CodeMirror-hint' + (i ? '' : ' iron-selected');
    ...
    function changeActive() {
      ...
      CodeMirror.signal(data, 'select', completions[i], node);
    }
    ...
    var ourMap, baseMap = {
      ...
      PageUp: function() {
        container.selected = (selectedHint - screenAmount());
        changeActive();
      },
      PageDown: function() {
        container.selected = (selectedHint + screenAmount());
      ...
      End: function() {
        container.selected = (completions.length - 1);
        changeActive();
      },
      Enter: pick,
      Tab: pick,
    ...
    function onBlur() {
      closingOnBlur = setTimeout(close, 100);
    };
    ...
    CodeMirror.on(hints, 'dblclick', function(e) {
      ...
      if (t.hintId != null) {
        selectedHint = t.hintId;
        container.selected = t.hintId;
        pick();
      }
    });
    ...
    var once, lastPos = cm.getCursor(),
      lastLen = cm.getLine(lastPos.line).length;

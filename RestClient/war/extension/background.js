 ...
 * Filter responses by it's URL to filter applications request URL.
 * @param details
 */
...
/**
 * Called when request's headers has been sent.
 * It filter requests by current set URL. After request filter URL is cleared.
 * @param details
 */
 ...
 * @param details:  message - (any data) The message sent by the calling script.
 * 					sender - (object): tab - This property will only be present when the connection was opened from a tab or content script; id - The extension ID of the extension that opened the connection.
 * 					sendResponse - (function) Function to call (at most once) when you have a response. The argument should be any JSON-ifiable object.
 * 
 *  To run application from external extension just pass message to it.
 *  
 *  For message description go to the end of this file.
 *  
 *  In response callback it will return object with one of values:
 *  "success": [true|false],
...
chrome.extension.onMessageExternal.addListener(function(message,sender,sendResponse) {
	...
	if(!message.payload||availablePayload.indexOf(message.payload)==-1){
		sendResponse({"error":true,"message":"Unknown payload"});
		return true;
	}
	...
	case 'create':
		if(!message.data){
			sendResponse({"error":true,"message":"Required data parameter is not available."});
			return;
		}
 ...
 *
 * "payload" (required) - message payload: "create" to open new application window with values; (other payloads may be available in future).
 *  "data" - (required) javascript object with any of values:
 *  	url - (String) request URL to set
 *  	method - (String) request method to set
 *  	headers - (String) an RFC string representing request headers
 *  			example: >>>

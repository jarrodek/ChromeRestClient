...
arc.app.db._dbUpgrade = function (e) {
    ...
    var headersstore = db.createObjectStore("headers", {keyPath: 'id'});
    headersstore.createIndex('name', 'name', {unique: false});
    headersstore.createIndex('type', 'type', {unique: false});
    headersstore.createIndex('name-type', ['name', 'type'], {unique: true});

    console.info('Creating "statuses" store and indexes.');
    //index for code (status code for search), must be unique
    var statusesstore = db.createObjectStore("statuses", {keyPath: 'id'});
    statusesstore.createIndex('code', 'code', {unique: true});

    console.info('Creating "autofillurls" store and indexes.');
    //index for url (historical for search), must be unique
    var autofillurlsstore = db.createObjectStore("autofillurls", {keyPath: 'id'});
    autofillurlsstore.createIndex('url', 'url', {unique: true});
    ...
    historystore.createIndex('store', 'store', {unique: false});
    historystore.createIndex('url-method', ['url', 'method'], {unique: true});

...
import com.kalicinscy.web.restclient.client.api.OAuth;
...
public class AuthorizeDialog extends DialogBox {
	...
	public AuthorizeDialog() {
		...
		buttonsPanel.setCellWidth(okButton, "70");
		...
		if( schemePos != -1 ){
			int authorityPos = url.indexOf("/", schemePos+3);
			if(authorityPos != -1){
				currentURL = url.substring(0,authorityPos +1).toLowerCase();
			}
		}
		...
		oauth_http_method = new Label(RestClient.REST_PARAMS.getMethod());
	...
	protected void setOauthValue() {
		String URL = RestClient.REST_PARAMS.getUrl();
		Map<String, String> oauthParams = getOauthParams();
		List<OauthParam> params = OAuth.createParamsList(oauthParams);
		String signature = OAuth.getSignatureValue(RestClient.REST_PARAMS.getMethod(), URL, params);
